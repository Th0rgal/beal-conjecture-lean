...
def validate(data:dict[str,Any],ray:dict[str,Any])->str:
    ...
    tests={'17_i':(17,base.I),'17_minus_i':(17,-base.I),'19_plus':(19,base.F49(1)),'19_minus':(19,base.F49(-1)),'11_plus':(11,base.F49(1)),'71_minus':(71,base.F49(-1)),'13_minus':(13,base.F49(-1))}
    ...
    if split['norm']!=45 or not all(nonsquare(v,p) for p,values in ((11,split['prime11_residues']),(19,split['prime19_residues'])) for v in values): raise CertificateError('nodal square-class mismatch')
    ...
    if conclusion['unique_reducibility_character']!='psi_(2,0)' or conclusion['forced_divisibility']!='19*71 divides C' or conclusion['additional_divisibility']!='13 divides A*C': raise CertificateError('global conclusion mismatch')
    return data['certificate_sha256']

def expect_rejection(data,ray,label):
    data['certificate_sha256']=digest(data)
    try: validate(data,ray)
    except CertificateError:return
    raise RuntimeError(f'checker accepted {label}')
def self_test():
    data,ray=load(MANIFEST),load(RAY);validate(data,ray)
    bad=copy.deepcopy(data);bad['ray_coordinates']['19a']['coordinate']=[3,0];expect_rejection(bad,ray,'a mutated ray coordinate')
    bad=copy.deepcopy(data);bad['nodal_splitting_character']['prime11_residues'][0]=1;expect_rejection(bad,ray,'a square nodal residue')
...
def main():
    ...
    value=validate(load(MANIFEST),load(RAY));print('global fixed-7 reducibility sieve valid');print('  unique ray character: psi_(2,0)');print('  19*71 divides C and 13 divides A*C');print(f'  certificate sha256: {value}');return 0
