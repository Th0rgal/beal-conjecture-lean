...
def validate(data:dict[str,Any])->str:
    ...
    if global_ray['conclusion']['forced_divisibility']!='19*71*79*89*131 divides C' or rational['conclusion']['forced_divisibility']!='13*29 divides C' or hasse['even_branch_corollary']['conclusion']!='7 divides C':raise CertificateError('input divisibilities changed')
    primes=data['prime_factors_for_C']; expected=[2,3,5,7,13,19,29,71,79,89,131]
    if primes!=expected or math.prod(primes)!=data['forced_divisor_of_C'] or data['forced_divisor_of_C']!=98369618750130:raise CertificateError('composed divisor mismatch')
...
def self_test():
    ...
    bad=copy.deepcopy(data);bad['prime_factors_for_C'].remove(79);bad['forced_divisor_of_C']//=79;expect_rejection(bad,'a weakened divisor')
...
def main():
    ...
    value=validate(load(MANIFEST));print('even rational global divisibility certificate valid');print('  98,369,618,750,130 divides C');print('  41 does not divide B');print(f'  certificate sha256: {value}');return 0
